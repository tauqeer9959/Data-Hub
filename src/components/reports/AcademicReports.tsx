...
export function AcademicReports() {
  ...
  const reportData = useMemo(() => {
    ...
    const cgpa = calculateCGPA(realData.semesters.map(sem => ({
      subjects: [],
      totalCreditHours: sem.credits,
    ...
    const gradeDistribution = realData.subjects.reduce((acc, subject) => {
      if (subject.grade && subject.grade !== 'N/A' && subject.grade.trim() !== '') {
    ...
    const completedSubjects = realData.subjects.filter(s => s.grade && s.grade !== 'N/A' && s.grade !== 'F' && s.grade !== '');
    const completionRate = realData.subjects.length > 0 ? (completedSubjects.length / realData.subjects.length) * 100 : 0;
    ...
    const strongSubjects = realData.subjects
      .filter(s => s.grade && gradePoints[s.grade] >= 3.3)
      .map(s => s.name)
      .slice(0, 5);

    const weakSubjects = realData.subjects
    ...
    return {
      overview: {
        cgpa,
        totalCredits: realData.semesters.reduce((sum, sem) => sum + sem.credits, 0),
        totalSubjects: realData.subjects.length,
        totalProjects: realData.projects.length,
        totalCertificates: realData.certificates.length,
        completionRate: Math.round(completionRate * 10) / 10
      },
      performance: {
        semesterProgress: realData.semesters,
      ...
      achievements: {
        recentGrades: realData.subjects.slice(-5),
        completedProjects: realData.projects.filter(p => p.status === 'completed'),
        earnedCertificates: realData.certificates
      }
    };
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{reportData.overview.cgpa.toFixed(2)}</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{reportData.overview.totalCredits}</div>
                <p className="text-xs text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{reportData.overview.completionRate}%</div>
                <p className="text-xs text-muted-foreground">
                  Program completion
                </p>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {reportData.overview.totalProjects + reportData.overview.totalCertificates}
                </div>
                <p className="text-xs text-muted-foreground">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-3">
                {reportData.achievements.recentGrades.map((subject, index) => (
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                {reportData.performance.strongAreas.map((area, index) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <DataVisualization
              config={{
                ...
                data: reportData.performance.gradeDistribution,
                type: 'pie',
                height: 300,
                showLegend: true,
                showTooltip: true
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(reportData.performance.gradeDistribution || {}).map(([grade, count]) => (
                      <div key={grade} className="flex items-center justify-between">
                        ...
                        <div className="w-24 bg-muted rounded-full h-2">
                          <div 
                            ...
                            style={{ 
                              width: `${(count / Math.max(...Object.values(reportData.performance.gradeDistribution || {}))) * 100}%` 
                            }}
                          />
                        </div>
                      </div>
