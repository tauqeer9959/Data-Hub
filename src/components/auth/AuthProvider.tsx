...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const createProfile = async (user: User, accessToken: string) => {
    try {
      ...
      // Extract user data from Google OAuth metadata with better fallbacks
      const googleUserData = user.user_metadata || {};
      const fullName = googleUserData.full_name || 
                      googleUserData.name || 
      ...
      console.log('Creating profile for Google user:', { fullName, email: user.email, profilePicture });
      ...
      const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-0603cad1/profile`, {
        ...
        body: JSON.stringify({
          ...
          phone: '',
          location: '',
          bio: `Joined via Google Sign-In on ${new Date().toLocaleDateString()}`,
          profilePicture,
          publicProfile: false,
          role: 'user'
        }),
      ...
      if (response.ok) {
        ...
        console.log('Profile created successfully for Google user:', profileData.fullName);
        return profileData;
      } else {
      ...
      // Always return a fallback profile for Google users
      const googleUserData = user.user_metadata || {};
      const fallbackProfile = {
        id: user.id,
        email: user.email || '',
        fullName: googleUserData.full_name || 
                 googleUserData.name || 
                 user.email?.split('@')[0] || 
                 'User',
        phone: '',
        location: '',
        bio: '',
        profilePicture: googleUserData.picture || googleUserData.avatar_url || '',
        publicProfile: false,
        role: 'user' as const,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString()
  ...
  const fetchProfile = async (userId: string, accessToken: string, user?: User) => {
    try {
      ...
      if (error.name === 'AbortError') {
        ...
        // Don't set error for timeout on initial load, just log it
        if (retryCount > 0) {
  ...
  useEffect(() => {
    ...
    }, 15000); // 15 seconds max for initial load
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        if (session?.user && session.access_token) {
          // Don't await profile fetch to avoid blocking auth state updates
          fetchProfile(session.user.id, session.access_token, session.user);
        } else {
          setProfile(null);
        }
