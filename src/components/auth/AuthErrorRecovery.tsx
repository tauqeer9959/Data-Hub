...
export function AuthErrorRecovery({ error, onRetry, onClear, onGoHome }: AuthErrorRecoveryProps) {
  const isGoogleAuthError = error.toLowerCase().includes('google') || 
          ...
          <div className="flex-1 mr-4">
            ...
            {isGoogleAuthError && (
              <div className="text-xs mt-2 text-muted-foreground">
                This may be a temporary issue with Google Sign-In. Try refreshing or signing in with email/password.
              </div>
            )}
          </div>
